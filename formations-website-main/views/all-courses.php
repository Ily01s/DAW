...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    //USER'S COURSES
    $commandeSql = "SELECT c.id FROM user_courses AS uc 
    ...
    $userCourses = $reponse->fetchAll(PDO::FETCH_COLUMN, 0);

} else if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['delete-course'])) { //DELETE COURSE
        $donnees['course-id'] = $_POST['course-id'];

        $commandeSql = "DELETE FROM user_courses WHERE idCour = ".$donnees['course-id'].";";
        try {$bdd->exec($commandeSql);}
        catch (PDOException $e) {echo "Erreur : " . $e->getMessage();die("");}

        $commandeSql = "DELETE FROM cour WHERE id = ".$donnees['course-id'].";";
        try {$bdd->exec($commandeSql);}
        catch (PDOException $e) {echo "Erreur : " . $e->getMessage();die("");}

    } else { //ADD COURSE TO USER'S LIST
        $donnees['course-id'] = $_POST['course-id'];
    ...
    <aside class="Z9A69u">
        ...
        <?php if ($utilisateur['role'] == 'etudiant') {?>
            <div class="VwBabc">
                ...
                <a href="all-courses.php" class="tjJOER active">
                    <div class="pHqVuN">
